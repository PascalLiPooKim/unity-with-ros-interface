...
plt.bar(candidates_num-0.2, total_score_3FAS, width=0.2, color='b', align='center' , alpha=0.5)
plt.bar(candidates_num, total_score_5FAS, width=0.2, color='g', align='center', alpha=0.5)
plt.bar(candidates_num+0.2, total_score_3DAS, width=0.2, color='r', align='center', alpha=0.5)
...
#plt.hist(x, n_bins, density=True, histtype='bar', color=colors, label=metrics)
plt.bar(np.array(scores_3FAS)-0.2, [int(count_3FAS[i]) for i in count_3FAS], width=0.2, color='b', align='center', alpha=0.5)
...
plt.bar(scores_5FAS, [int(count_5FAS[i]) for i in count_5FAS], width=0.2, color='g', align='center', alpha=0.5)
plt.bar(np.array(scores_3DAS)+0.2, [int(count_3DAS[i]) for i in count_3DAS], width=0.2, color='r', align='center', alpha=0.5)
plt.legend(prop={'size': 10})
...
plt.barh(range(1, 9), [4, 5, 8, 8, 9, 9, 10, 11], color=['r', 'r', 'g', 'r', 'g', 'r', 'g', 'g'], align='center', alpha=0.5)
plt.yticks(range(1, 9), [7, 6, 2, 8, 4, 3, 5, 1])
plt.xlabel("SAGAT Total Score")
plt.ylabel("Candidate Number")
...
NASATLX_path = r"D:\GitHub\unity-with-ros-interface\Graphs\NASA TLX Course 3.csv"
...
new_df_plot.set_ylabel("Avegare Point", fontsize=30)
...
for autotxt in autotxts:
    autotxt.set_color('white')
    autotxt.set_fontsize(40)
plt.title("Percentages of candidates with a certain Gaming Experience")


plt.subplot(1, 2, 2)
labels = ['Robotics/Teleoperation Expertise', 'NO Robotics/Teleoperation Expertise']
_, _, autotxts = plt.pie([3, 2], labels =labels, autopct='%1.1f%%', shadow = True, colors=['c', 'y'])
for autotxt in autotxts:
    autotxt.set_color('white')
    autotxt.set_fontsize(40)
plt.title("Percentages of candidates having Robotics/Teleoperation Expertise")
plt.show()
...
NASATLX_path = r"D:\GitHub\unity-with-ros-interface\Graphs\NASA TLX Course 2.csv"
df = pd.read_csv(NASATLX_path, engine='python')
#print(df.dtypes)
print(df["Mental Demand: How mentally demanding was the task?.1"])
NASA_TLX_VA = df[["Mental Demand: How mentally demanding was the task?", 
...
new_df_plot.set_yerr = new_df.Errors_VA.to_frame("Mean of Vision & Audio")
new_df_plot.set_ylabel("Avegare Point", fontsize=30)
new_df_plot.set_title("NASA Task Load Index of Deteriorated Vision Experiment")
...
plt.title(r'Mean of Completion Time')
...
plt.xticks(np.arange(2), ['Vision & Audio', 'Vision'])
plt.title(r'Mean of # Keystrokes')
...
plt.xticks(np.arange(2), ['Vision & Audio', 'Vision'])
plt.title(r'Mean of # Collisions')
plt.show()
...
driving_exp_path = r"D:\GitHub\unity-with-ros-interface\Graphs\Husky Driving Experience.csv"
...
mean_plot = driving_exp_VA.mean().plot(kind='bar', color =['r', 'b', 'y', 'g'], rot=0)


mean_plot.set_xticklabels(["How much did you like \n driving the Husky with \n visual feedback only?",
                           "How much did you like \n driving the Husky with both \n visual feedback and auditory cues?",
                           "How useful do you think \n are the auditory cues?",
                           "How useful do you think \n are the auditory \ncues for driving around?"])
